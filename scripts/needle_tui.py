...
def render(sess):
    ...
    foot = Layout()
    foot.update(Panel(stats, title="[grey58]stats[/]", border_style="grey30"))
...
def main():
    ...
    try:
        while True:
            ...
            # The model has finished generating and returned a validated JSON tool call.
            # Now we can parse it and run normal Python code!
            call = sess.call_json()
            if isinstance(call, list) and len(call) > 0:
                ...
                # Make sure Python can find the tools/ module from inside scripts/
                import sys, os
                ...
                action, result = dispatcher.dispatch(name, args)
                
                CONSOLE.print(f"🔧 [bold cyan]SYSTEM ACTION:[/] {action}")
